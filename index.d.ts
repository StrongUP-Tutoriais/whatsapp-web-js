...
declare namespace WAWebJS {
    ...
    export class Client extends EventEmitter {
        ...
        /** Página do Puppeteer rodando o WhatsApp Web */
        pupPage?: puppeteer.Page

        /** Navegador do Puppeteer rodando o WhatsApp Web */
        pupBrowser?: puppeteer.Browser

        /** Interface de interatividade do cliente */
        interface?: InterfaceController

        /** Aceita um convite para entrar em um grupo */
        acceptInvite(inviteCode: string): Promise<string>

        /** Aceita um convite privado para entrar em um grupo (convite v4) */
        acceptGroupV4Invite: (inviteV4: InviteV4Data) => Promise<{ status: number }>

        /** Retorna um objeto com informações sobre o grupo do código de convite */
        getInviteInfo(inviteCode: string): Promise<object>

        /** Habilita e retorna o estado de arquivamento do chat */
        archiveChat(chatId: string): Promise<boolean>

        /** Fixar o chat e retorna seu novo estado de fixação */
        pinChat(chatId: string): Promise<boolean>

        /** Desfixar o chat e retorna seu novo estado de fixação */
        ...
        /** Faz o logout do cliente, fechando a sessão atual */
        logout(): Promise<void>

        /** Obtém todos os contatos bloqueados pela conta principal */
        getBlockedContacts(): Promise<Contact[]>

        /** Obtém uma instância de chat pelo ID */
        getChatById(chatId: string): Promise<Chat>

        /** Obtém todas as instâncias de chat atuais */
        getChats(): Promise<Chat[]>

        /** Obtém uma instância de contato pelo ID */
        getContactById(contactId: string): Promise<Contact>

        /** Obtém uma mensagem pelo ID */
        getMessageById(messageId: string): Promise<Message>

        /** Obtém todas as instâncias de contato atuais */
        getContacts(): Promise<Contact[]>

        /** Obtém o código do país de um ID do WhatsApp. (154185968@c.us) => (1) */
        getCountryCode(number: string): Promise<string>

        /** Obtém o número formatado de um ID do WhatsApp. (12345678901@c.us) => (+1 (234) 5678-901) */
        getFormattedNumber(number: string): Promise<string>

        /** Obtém todos os rótulos atuais */
        getLabels(): Promise<Label[]>

        /** Altera rótulos nos chats */
        addOrRemoveLabels(labelIds: Array<number | string>, chatIds: Array<string>): Promise<void>

        /** Obtém uma instância de rótulo pelo ID */
        getLabelById(labelId: string): Promise<Label>

        /** Obtém todos os rótulos atribuídos a um chat */
        getChatLabels(chatId: string): Promise<Label[]>

        /** Obtém todos os chats para um rótulo específico */
        getChatsByLabelId(labelId: string): Promise<Chat[]>

        /** Retorna a URL da foto de perfil do contato, se as configurações de privacidade permitirem */
        getProfilePicUrl(contactId: string): Promise<string>

        /** Obtém os grupos comuns do contato com você. Retorna um array vazio se você não tiver nenhum grupo comum. */
        getCommonGroups(contactId: string): Promise<ChatId[]>

        /** Obtém o estado atual da conexão para o cliente */
        getState(): Promise<WAState>

        /** Retorna a versão do WhatsApp Web que está sendo executada */
        getWWebVersion(): Promise<string>

        /** Configura eventos e requisitos, inicia o pedido de autenticação */
        initialize(): Promise<void>

        /** Verifica se um ID dado está registrado no WhatsApp */
        isRegisteredUser(contactId: string): Promise<boolean>

        /** Obtém o ID registrado do WhatsApp para um número. Retorna null se o número não estiver registrado no WhatsApp. */
        getNumberId(number: string): Promise<ContactId | null>

        /**
         * Silencia este chat para sempre, a menos que uma data seja especificada
         * @param chatId ID do chat que será silenciado
         * @param unmuteDate Data quando o chat será desmutado, deixe como está para silenciar para sempre
         */
        muteChat(chatId: string, unmuteDate?: Date): Promise<void>

        /**
         * Solicita autenticação via código de emparelhamento em vez de código QR
         * @param phoneNumber - Número de telefone no formato internacional, sem símbolos (ex.: 12025550108 para os EUA, 551155501234 para o Brasil)
         * @param showNotification - Mostrar notificação para emparelhar no número de telefone
         * @returns {Promise<string>} - Retorna um código de emparelhamento no formato "ABCDEFGH"
         */
        requestPairingCode(phoneNumber: string, showNotification = true): Promise<string>

        /** Força o reset do estado de conexão para o cliente */
        resetState(): Promise<void>

        /** Envia uma mensagem para um chat específico (chatId) */
        sendMessage(chatId: string, content: MessageContent, options?: MessageSendOptions): Promise<Message>

        /** Pesquisa por mensagens */
        searchMessages(query: string, options?: { chatId?: string, page?: number, limit?: number }): Promise<Message[]>

        /** Marca o cliente como online */
        sendPresenceAvailable(): Promise<void>

        /** Marca o cliente como offline */
        sendPresenceUnavailable(): Promise<void>

        /** Marca como visto para o chat */
        ...
        /** 
         * Define a mensagem de status do usuário atual
         * @param status Nova mensagem de status
         */
        setStatus(status: string): Promise<void>

        /** 
         * Define o nome exibido do usuário atual
         * @param displayName Novo nome exibido
         */
        setDisplayName(displayName: string): Promise<boolean>

        /**
         * Altera o auto-download de Áudio
         * @param flag true/false para ativar ou desativar
         */
        setAutoDownloadAudio(flag: boolean): Promise<void>

        /**
         * Altera o auto-download de Documentos
         * @param flag true/false para ativar ou desativar
         */
        setAutoDownloadDocuments(flag: boolean): Promise<void>

        /**
         * Altera o auto-download de Fotos
         * @param flag true/false para ativar ou desativar
         */
        setAutoDownloadPhotos(flag: boolean): Promise<void>

        /**
         * Altera o auto-download de Vídeos
         * @param flag true/false para ativar ou desativar
         */
        setAutoDownloadVideos(flag: boolean): Promise<void>

        /**
         * Obtém a contagem de dispositivos do usuário pelo ID
         * Cada Conexão WaWeb conta como um dispositivo, e o telefone (se existir) conta como um
         * Então, para um usuário não empresarial com uma conexão WaWeb, deve retornar "2"
         * @param {string} contactId
         */
        getContactDeviceCount(contactId: string): Promise<Number>

        /** Altera e retorna o estado de arquivamento do chat */
        unarchiveChat(chatId: string): Promise<boolean>

        /** Desativa o mudo do chat */
        unmuteChat(chatId: string): Promise<void>

        /** Define a foto de perfil do usuário atual */
        setProfilePicture(media: MessageMedia): Promise<boolean>

        /** Deleta a foto de perfil do usuário atual */
        deleteProfilePicture(): Promise<boolean>

        /** Obtém uma lista de solicitações de adesão */
        getGroupMembershipRequests: (groupId: string) => Promise<Array<GroupMembershipRequest>>

        /** Aprova as solicitações de adesão, se houver */
        ...
        /** Rejeita as solicitações de adesão, se houver */
        ...
        /** Emite quando há uma falha na autenticação ao tentar restaurar uma sessão existente */
        on(event: 'auth_failure', listener: (message: string) => void): this

        /** Emite quando a autenticação é bem-sucedida */
        on(event: 'authenticated', listener: (
            /** 
             * Objeto contendo informações da sessão, ao usar LegacySessionAuth. Pode ser usado para restaurar a sessão
             */
            session?: ClientSession
        ) => void): this

        /** 
         * Emite quando a porcentagem da bateria do dispositivo conectado muda
         * @deprecated 
         */
        on(event: 'change_battery', listener: (batteryInfo: BatteryInfo) => void): this

        /** Emite quando o estado da conexão muda */
        on(event: 'change_state', listener: (
            /** o novo estado da conexão */
            state: WAState
        ) => void): this

        /** Emite quando o cliente foi desconectado */
        on(event: 'disconnected', listener: (
            /** motivo que causou a desconexão */
            reason: WAState | "LOGOUT"
        ) => void): this

        /** Emite quando um usuário entra no chat via link de convite ou é adicionado por um administrador */
        on(event: 'group_join', listener: (
            /** Notificação do grupo com mais informações sobre a ação */
            notification: GroupNotification
        ) => void): this

        /** Emite quando um usuário sai do chat ou é removido por um administrador */
        on(event: 'group_leave', listener: (
            /** Notificação do grupo com mais informações sobre a ação */
            notification: GroupNotification
        ) => void): this

        /** Emite quando um usuário atual é promovido a administrador ou rebaixado a usuário comum */
